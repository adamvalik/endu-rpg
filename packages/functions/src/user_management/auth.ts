...
export const signUpOrLogIn = onCall(async (request: CallableRequest<SignUpOrLogInData>) => {
  ...
  try {
    ...
    if (isNewUser) {
      ...
      await db.collection(FIRESTORE_COLLECTIONS.USERS).doc(userRecord.uid).set(userProfile);
    ...
    handleError(error, 'Error in signUpOrLogIn:', 'Failed to sign up or sign in user.');
  }
});
...
export const onUserDelete = functionsV1.auth.user().onDelete(async (user) => {
  ...
  try {
    ...
    await db.collection(FIRESTORE_COLLECTIONS.STRAVA_TOKENS).doc(user.uid).delete();
